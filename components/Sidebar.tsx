    ...
    <aside className={`bg-[#1F1F1F] text-white flex flex-col shadow-2xl transition-all duration-300 z-40 ${isCollapsed ? 'w-20' : 'w-64'}`}>
         ...
         {!isCollapsed ? (
           <div className="flex items-center space-x-3 overflow-hidden">
             <div className="w-10 h-10 bg-white rounded-xl overflow-hidden shrink-0 border-2 border-[#8B0000]">
               <img src="https://i.postimg.cc/GHPqRnpw/LOGO-SISTEMMA.jpg" alt="Logo" className="w-full h-full object-cover" />
             </div>
      ...
      <nav className="flex-1 mt-6 px-3 space-y-1 overflow-y-auto scrollbar-hide">
        {menuItems.map((item, idx) => {
          ...
          return (
            <button key={item.id} onClick={() => setView(item.id as ViewType)} className={`w-full flex items-center ${isCollapsed ? 'justify-center' : 'space-x-3'} px-4 py-4 rounded-2xl transition-all ${isActive ? 'bg-[#8B0000] text-white shadow-[0_10px_20px_-5px_rgba(139,0,0,0.5)]' : 'text-gray-400 hover:bg-gray-800 hover:text-white'}`}>
