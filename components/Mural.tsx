...
import { 
  ...
  Landmark, ArrowUpRight, ChevronDown, X 
} from 'lucide-react';
...
const Mural: React.FC<MuralProps> = ({ messages, user, onInteraction }) => {
  ...
  const handleDelete = async (id: string) => {
    if (confirm("Deseja remover esta corrente de discussão?")) {
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      <div className="flex items-center justify-between">
        ...
        <button 
          ...
          className="bg-[#1F1F1F] text-white px-8 py-3.5 rounded-2xl flex items-center space-x-3 font-black text-[10px] uppercase tracking-widest shadow-xl"
        >
          <Plus size={18} />
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
              <label className="flex items-center space-x-3 cursor-pointer">
                 <input type="checkbox" checked={important} onChange={e => setImportant(e.target.checked)} className="w-5 h-5 accent-[#8B0000]" />
      ...
      <div className="grid grid-cols-1 gap-6">
        {messages.map(msg => {
          ...
          return (
            <div key={msg.id} className={`bg-white rounded-[2.5rem] shadow-xl border-2 ${msg.status === MuralStatus.CRITICO ? 'border-red-500' : 'border-gray-50'} overflow-hidden`}>
              <div className="p-8 md:p-10">
                <div className="flex justify-between items-start mb-6">
                   <div className="flex items-center space-x-4">
                      <div className={`w-12 h-12 ${config.bg} rounded-2xl flex items-center justify-center ${config.color} font-black text-lg`}>
                   ...
                   {/* TRAVA DE EXCLUSÃO: SÓ O CRIADOR VÊ O BOTÃO */}
                   {isOwner && (
                ...
                <div className="bg-gray-50/50 p-6 rounded-2xl text-sm text-gray-700 font-bold leading-relaxed mb-8">
                   {msg.content}
                </div>
                ...
                <div className="flex items-center justify-between pt-6 border-t border-gray-50">
                   <button 
                     ...
                     className="flex items-center space-x-3 text-[10px] font-black uppercase text-gray-400 hover:text-[#8B0000]"
                   >
                   ...
                   <div className="flex-1 max-w-lg ml-8 flex space-x-2">
                      <input 
                        type="text" 
                        className="flex-1 bg-gray-50 border border-gray-100 rounded-xl px-4 py-2 text-[10px] font-bold outline-none" 
                        placeholder="Responder a esta discussão..."
                        value={replyText[msg.id] || ''}
                        onChange={e => setReplyText({...replyText, [msg.id]: e.target.value})}
                        onKeyDown={e => e.key === 'Enter' && handleReply(msg.id)}
                      />
                      <button onClick={() => handleReply(msg.id)} className="p-2 bg-[#8B0000] text-white rounded-xl"><Send size={14} /></button>
                   </div>
                </div>

                {expandedId === msg.id && (
                  <div className="mt-8 space-y-4 pt-8 border-t border-gray-100 animate-in slide-in-from-bottom-2">
