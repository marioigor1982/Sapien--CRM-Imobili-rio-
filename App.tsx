    ...
    <div className="flex h-screen bg-[#F3F4F6] overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 relative">
        ...
        <main className="flex-1 overflow-auto bg-white">
          ...
          {currentView === 'Clientes' && <div className="p-8 bg-[#F3F4F6] min-h-full"><GenericCrud title="Clientes" data={clients} type="client" onSave={d => d.id ? clientService.update(d.id, d) : clientService.create(d)} onDelete={clientService.remove} isAdmin={isAdmin} brokers={brokers} properties={properties} banks={banks} companies={companies} companyService={companyService} propertyService={propertyService} brokerService={brokerService} bankService={bankService} /></div>}
          {currentView === 'Corretores' && <div className="p-8 bg-[#F3F4F6] min-h-full"><GenericCrud title="Corretores" data={brokers} type="broker" onSave={d => d.id ? brokerService.update(d.id, d) : brokerService.create(d)} onDelete={brokerService.remove} isAdmin={isAdmin} leads={leads} properties={properties} brokerService={brokerService} /></div>}
          {currentView === 'Properties' && <div className="p-8 bg-[#F3F4F6] min-h-full"><GenericCrud title="Imóveis" data={properties} type="property" onSave={d => d.id ? propertyService.update(d.id, d) : propertyService.create(d)} onDelete={propertyService.remove} isAdmin={isAdmin} companies={companies} companyService={companyService} propertyService={propertyService} /></div>}
          {currentView === 'Bancos' && <div className="p-8 bg-[#F3F4F6] min-h-full"><GenericCrud title="Bancos" data={banks} type="bank" onSave={d => d.id ? bankService.update(d.id, d) : bankService.create(d)} onDelete={bankService.remove} isAdmin={isAdmin} bankService={bankService} /></div>}
          {currentView === 'Construtoras' && <div className="p-8 bg-[#F3F4F6] min-h-full"><GenericCrud title="Construtoras" data={companies} type="company" onSave={d => d.id ? companyService.update(d.id, d) : companyService.create(d)} onDelete={companyService.remove} isAdmin={isAdmin} companyService={companyService} /></div>}
        </main>
